...
public class NationalityRegexTest extends AnnotatorTestBase{
  ...
  @Before
	public void beforeTest() throws UIMAException{
		...
		final ExternalResourceDescription erd = ExternalResourceFactory.createExternalResourceDescription("country", SharedCountryResource.class);
		final AnalysisEngineDescription aed = AnalysisEngineFactory.createEngineDescription(NationalityRegex.class, "country", erd);
	...
	public void test() throws Exception{
		...
		final Nationality british = JCasUtil.selectByIndex(jCas, Nationality.class, 0);
		...
		final Nationality irish = JCasUtil.selectByIndex(jCas, Nationality.class, 1);
		assertNotNull(irish);
		assertEquals("Irish", irish.getCoveredText());
		assertEquals("Irish", irish.getValue());
		assertEquals("IRL", irish.getCountryCode());
		
		final Nationality spanish = JCasUtil.selectByIndex(jCas, Nationality.class, 2);
...
ae.process(jCas);
        ...
        final Nationality irish = JCasUtil.selectByIndex(jCas, Nationality.class, 0);
        assertNotNull(irish);
        assertEquals("Irish", irish.getCoveredText());
        assertEquals("Irish", irish.getValue());
        assertEquals("IRL", irish.getCountryCode());
        
        final Nationality spanish = JCasUtil.selectByIndex(jCas, Nationality.class, 1);
        assertNotNull(spanish);
        assertEquals("Spanish", spanish.getCoveredText());
        assertEquals("Spanish", spanish.getValue());
        assertEquals("ESP", spanish.getCountryCode());
