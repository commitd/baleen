...
public class DataAttributeMapper implements ContentMapper {
  ...
  public void map(final JCas jCas, final Element element, final AnnotationCollector collector) {
    final Set<String> types = MarkupUtils.getTypes(element);
    ...
    for (final String type : types) {
      try {
        ...
        Arrays.stream(annotation.getClass().getMethods())
            ...
            .forEach(m -> {
              setMethodValue(type, element, annotation, m);
            });
   ...
   * @param m the method
   */
  private void setMethodValue(final String type, final Element element, final Object annotation,
      final Method m) {
    String fieldName = m.getName().substring("set".length());
    fieldName = Character.toLowerCase(fieldName.charAt(0)) + fieldName.substring(1);
    final List<String> attributes = MarkupUtils.getAttributes(element, fieldName);
   ...
   */
  private void assignAttribute(final Object annotation, final Method m, final String string)
