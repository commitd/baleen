 ...
 * practise comments are usually single or multiple pararaph stretches.
 * 
  ...
  public void manipulate(final Document document) {
    document.select("p:contains(" + COMMENT_END + ")").forEach(last -> {
      ...
      if (last.ownText().contains(COMMENT_START)) {
        ...
        final int index = last.elementSiblingIndex();
        final Elements allSiblings = last.siblingElements();
        final Elements pSiblings = allSiblings.select("p");
        Element startSibling = null;
        for (int i = index - 1; i >= 0; i--) {
          final Element e = pSiblings.get(i);
